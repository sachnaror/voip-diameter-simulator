...
│   ├── routes.py               # Defines API endpoints for health, config, call simulation, and Diameter simulation.
│   └── server.py               # Creates and configures the FastAPI application instance.
├── logs/                       # Stores generated call and Diameter log files.
├── tests/                      # Contains automated tests for API and simulator behavior.
├── .env.example                # Provides sample environment variables for local setup.
├── .gitignore                  # Excludes local env files, caches, and build artifacts from git.
├── requirements.txt            # Lists Python dependencies needed to run the simulator.
└── README.md                   # Documents the project overview, structure, and setup steps.
```
